...
class Template
{
	...
	{
		$this->file = $templateName;
		echo __DIR__."/".$this->arrayConfig['templateDir'].$templateName.".tpl";
		// 判断是否存在关联的模板文件
		if(file_exists(__DIR__."/".$this->arrayConfig['templateDir'].$templateName.".tpl"))
		{
			exit('找不到对应的模板');
		}
		$compileFile = $this->arrayConfig['compileDir'].md5($templateName).'.php';
		$cacheFile = $this->arrayConfig['compileDir'].md5($templateName).$this->arrayConfig['suffix_cache'];
		
		if($this->reCache($templateName) === false)
		{
			//如果需要缓存
